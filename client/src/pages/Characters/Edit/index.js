...
const EditCharacter = ({ showLogin, match: { params: { _id } = {} } = {}, history: { push, replace } = {}, location }) => {
  ...
  return (
    <Grid className="edit-character stuffs-list marg-b-3-rem" gap="3em" rowsTemplate="3.5em 4.5em" columnsTemplate={{ xs: '1fr', md: 'repeat(2, 1fr)', xl: 'repeat(3, 1fr)', xxxl: 'repeat(4, 1fr)' }}>
        ...
        { error ? `Oups! ${error.message}` : (
          <Column>
            <Row className="stuff-infos" row={1} col={1} width={6}>
              ...
              <Input className="level marg-v-0" type="number" style={{ flex: 2 }} value={ newLevel } onChange={(e) => setLevel(get(e, 'target.value') && parseInt(e.target.value, 10))} />
              <Input className="name marg-v-0" style={{ flex: 5 }} value={ newName } onChange={(e) => setName(get(e, 'target.value'))} />
