...
import { PetsTypes, translatePetsTypes } from '../constants/pets';
...
export const initLocalState = function(req, res, next) {
  setLocale(res, { typesList: PetsTypes, model: Equipment, translations: translatePetsTypes, toPopulate: { path: 'set', populate: { path: 'equipments', select: '-recipe -createdAt -updatedAt' } } })
  next();
}
...
export const get = function(req, res) {
    ...
    Equipment.findById(itemId).populate('set').exec((err, pet) => {
        if (err) res.send(err);
        res.json(pet);
    });
};
